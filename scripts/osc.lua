...
local user_opts = {
    ...
    title = "${!playlist-count==1:[${playlist-pos-1}/${playlist-count}] }${?metadata/by-key/track:${metadata/by-key/track} - }${media-title}",
...
local function window_controls(topbar)
    ...
    if track_number ~= "" then
        title = title:gsub("^%[(%d+)/(%d+)%] ", function(pos, count)
            return string.format("[%s/%s] ", pos, count)
        end)
        title = title:gsub(track, track_number, 1)
    end
...
local function osc_init()
    ...
    ne.content = function ()
        if state.forced_title then
            return mp.command_native({"escape-ass", state.forced_title})
        end
        ...
        local track = mp.get_property("metadata/by-key/track") or ""
        local track_num = tonumber(track:match("^%d+"))
        local track_number = track_num and string.format("%02d", track_num) or ""
        ...
        local title = clock .. " | "
        if track_number ~= "" then
            title = title .. track_number .. " "
        end
        title = title .. media_title
        if chapter_title ~= "" and ext:lower() ~= "m4a" then
